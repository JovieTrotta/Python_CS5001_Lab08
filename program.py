...
During this lab we read in data from a .csv file and ran a few calculations on it.
The calculations and organized data were then printed into a new .csv file. 
'''
...
def read_in(input_file):
    ...
    # Creates the Portland List 
    portland_data = [[],[],[],[],[],[],[],[],[],[],[],[]]

    # Creates the variable that will show how many blanks are in the data; sets it to zero
    times_except_block_hit = 0
    ...
    for line in input_contents.splitlines(): 
        ...
        # If list index[1] starts with "PORTLAND", continue
        if line_list[1].startswith("PORTLAND"): 
            ...
            # Goes through each year, adds year 2010 to portland_data[0], year 2011 to portland_data[1], and so on
            # If an error occurs, add one to the error block tally variable
            try:
                portland_data[int(year) - 2010].append(float(line_list[9]))
            except:
                times_except_block_hit += 1
    
    # Prints the number of times the error block was hit
    print("Number of blanks in data set: " + str(times_except_block_hit))

    # Saves the portland_data list for later use
    return portland_data
...
def print_avgs(data, city, starting_year, out_file):
    ...
    # Write data in the output file by looping through 2010-2021 and getting the average for every line in portland_data
        ...
        file_out.write(city + " Average Rainfall:\n\n")
    ...
    try:
        ...
        # Checks that the input file exists.
            ...
            # Checks that the output file exists.
                ...
                print_avgs(read_in(input_file),"Portland",2010,output_file)
